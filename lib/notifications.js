...
export async function requestNotificationPermission() {
  ...
  try {
    ...
    if (permission === 'granted') {
      toast.success('Notifications enabled. BrainMate may send optional learning reminders.');
      return true;
...
/**
 * Optional re-engagement notification
 * Triggers 30 minutes after a learner leaves the app, only after actual learning activity
 */
...
export function setupReengagementNotification() {
  ...
  const REENGAGE_DELAY_MS = 30 * 60 * 1000; // 30 minutes

  const handleVisibilityChange = () => {
    if (document.visibilityState === 'hidden') {
      // Only schedule a reminder after the user has actually completed a learning action.
      try {
        if (!localStorage.getItem('brainmate.last_learning_at')) return;
      } catch (e) {
        return;
      }
      // User exited or switched tabs: schedule a 30-minute re-engagement notification
